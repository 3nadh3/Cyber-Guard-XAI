...
> Built with **FastAPI + LIME + DistilBERT** (backend) and **React + Vite** (frontend).
...
```bash
git clone git@github.com:trinadhmusunuri12-pixel/backend.git cyber-guard-XAI
cd cyber-guard-XAI
```
...
POST /analyze
  ...
  ├── TF-IDF vectorizer      — transform to feature vector
  ├── Logistic Regression    — predict class + probability
  └── LIME explainer         — per-word influence scores (1000 samples)
       │
       ▼
Frontend renders colored tokens
  Red   = phishing signal (LIME score > 0)
  Green = safe signal     (LIME score < 0)
       │
       ▼  (if phishing detected)
POST /suggest
  ├── DistilBERT MLM         — fill-mask to get contextual candidates
  ├── Filter candidates      — exclude phishing keywords + stopwords
  └── Score each candidate   — re-run classifier, return risk delta
       │
       ▼
Adversarial Evasion Guide — user swaps words, watches % Phishing drop
```
...
| scikit-learn      | 1.4.2     | Logistic Regression + TF-IDF         |
| pandas            | 2.2.2     | Dataset loading                      |
| lime              | 0.2.0.1   | Local explainability (LIME)          |
| transformers      | latest    | DistilBERT MLM for word suggestions  |
...
> The first run downloads ~250MB of model weights. Ensure you have internet access and at least 1GB of free RAM. The app still works without it — evasion suggestions will be unavailable.
